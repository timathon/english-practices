...
/**
 * Vocab Master Release Generator (V2)
        ...
        allQuestions.forEach(q => {
            ...
            if (!uniqueSentencesMap.has(hash)) {
                if (audioMode === '3' || !existingHashes.has(hash)) {
                    uniqueSentencesMap.set(hash, q.context_sentence);
                }
            }
        });
    ...
    const getFilesFromFolder = (folderName) => {
        ...
        entries.forEach(entry => {
            ...
            if (stats.isDirectory()) {
                const subFiles = fs.readdirSync(fullPath).filter(f => f.endsWith('-vocab-master.json'));
                subFiles.forEach(subFile => {
                    results.push(path.join(entry, subFile));
                });
            } else if (entry.endsWith('-vocab-master.json')) {
                results.push(entry);
            }
        });
        return results;
...
if (args.length === 0) {
    ...
    if (inputJson && type && outputHtml) {
        ...
        console.error("Usage: node scripts/vm_release_gen-3.cjs [inputJson] [type] [outputHtml] [--user=10] [--validity=3]");
    }
}
