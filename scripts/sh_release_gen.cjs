...
const { getAudioBatch } = require('./tts-gen-cut.cjs');
...
async function generate(jsonPath, type, outputPath, userCount = 3, validityMonths = 3, audioMode = '1') {
    ...
    // Ensure total questions is divisible by 10
...
async function interactive() {
    ...
    if (selectedFolders.length === 1) {
        const allFiles = fs.readdirSync(path.join(dataDir, selectedFolders[0])).filter(f => f.endsWith('-spelling-hero.json'));
        ...
        selectedFolders.forEach(folder => filesToProcess.push(...fs.readdirSync(path.join(dataDir, folder)).filter(f => f.endsWith('-spelling-hero.json')).map(f => ({ folder, file: f }))));
    }
