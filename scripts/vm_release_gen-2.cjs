...
async function interactive() {
    ...
    if (selectedFolders.length === 1) {
        ...
        const allFiles = fs.readdirSync(folderPath).filter(f => f.endsWith('-vocab-master.json'));
        ...
        selectedFolders.forEach(folder => {
            ...
            const files = fs.readdirSync(folderPath).filter(f => f.endsWith('-vocab-master.json'));
