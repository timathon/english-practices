...
const SOURCE_PREFIX = "ep/b-think1-u11-p106-p109/";
...
async function main() {
    console.log(`Starting to move files from "${SOURCE_PREFIX}" to "${DEST_PREFIX}" in batches of 30...`);
    ...
    const keys = await listAll(SOURCE_PREFIX);
    ...
    for (let i = 0; i < filesToMove.length; i += batchSize) {
        ...
        const copyPromises = batch.map(key => {
            const filename = key.replace(SOURCE_PREFIX, "");
    ...
    console.log(`\n🎉 Done! All files moved successfully in batches of 30.`);
}
