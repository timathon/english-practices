...
**Sections:**
  ...
  - **A7A - A9 (including A7A, A7B, A8A, A8B, A9):** "Section A, 1b and 1c" (or "Section A, 1b, 1c, and 1d" etc., only if the first section of the listening scripts in the appendix is long and meaningful enough to include; otherwise skip this section), "Section A Activity 2a", "Section B Activity 1b", "Section B Activity 2a". **CRITICAL:** If "Reading Plus" (or "Unit X - Reading Plus") is present in the unit markdown file, you MUST also include "Reading Plus" as a section.
  ...
  - **PU1:** "The Friendly Farm" and "Literature".
    - **Proper Nouns & Translation Note for Power Up (PU1):** In the Power Up series, "Friendly" is the family name of the main characters (Jim & Jenny Friendly, the Friendly family). Therefore, **"The Friendly Farm"** is the family's farm name and must be translated as **"弗兰德利农场"** (NOT "友好的农场"). Similarly, "the Friendly family" is translated as "弗兰德利一家".
...
- **Additional Question Type Schemas:**
  ...
  6. **`cloze-passage-wordbank`** (Passage with inline blanks filled from a shared option/word pool; each option can be used once):
     ...
     - **任务型阅读 Conversion Rule:** Always map the section "任务型阅读" (Task-based Reading, which traditionally has open-ended or short-answer questions based on a passage) to the `"multiple-choice"` type under a `"reading-comprehension"` section (instead of short-answer), and generate 4 plausible multiple-choice options (`options` array) with one correct answer index (0-3) for each question using AI.
      - **Multi-Passage Reading Comprehension Rule:** When a reading comprehension section contains multiple distinct reading passages (e.g., labeled "### A", "### B", or "Passage A", "Passage B"), each passage MUST be represented as an independent section object in the `sections` array (e.g., `s2`: "二、阅读理解 A", `s3`: "三、阅读理解 B"), each with its own `id`, `title`, `instruction`, `type: "reading-comprehension"`, `passage` (containing only that passage's text), and its respective subset of `questions`. Do NOT combine or concatenate multiple distinct reading passages into a single shared section.
